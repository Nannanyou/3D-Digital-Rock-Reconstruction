    ...
    with tf.name_scope('ProcessReals'):
        ...
        if mirror_augment:
            with tf.name_scope('MirrorAugment'):
                ...
                x = tf.where(mask < 0.5, tf.reverse(x, axis=[3]), x)
                # x = tf.where(tf.logical_and(mask >= 0.25, mask < 0.5), tf.reverse(x, axis=[2]), x)
                # x = tf.where(tf.logical_and(mask >= 0.5, mask < 0.75), tf.reverse(tf.reverse(x, axis=[2]), axis=[3]), x)
...
def train_progressive_gan(
        ...
        D_repeats=1,
        # How many times the discriminator is trained per G iteration.
        minibatch_repeats=4,
        ...
        # Enable mirror augment?
        drange_net=[-1, 1],
        # Dynamic range used when feeding image data to the networks.
        image_snapshot_ticks=1,
        # How often to export image snapshots?
        network_snapshot_ticks=10,
        # How often to export network snapshots?
        save_tf_graph=False,
        # Include full TensorFlow computation graph in the tfevents file?
        ...
        # Snapshot index to resume training from, None = autodetect.
        resume_kimg=0.0,
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            # Report progress.
            print(
                'tick %-5d kimg %-8.1f lod %-5.2f minibatch %-4d time %-12s sec/tick %-7.1f sec/kimg %-7.2f '
                'maintenance %.1f' % (
